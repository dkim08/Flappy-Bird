...
const Pipe: React.FC<PipeProps> = ({ position, size }: PipeProps): ReactElement => {
    ...
    return (
        <div className={`${size}-pipe-wrapper`} style={{ ...transformStyles, }}>
            <img className={`${size}-pipe`} src={imgSrc} alt="" />
        </div>
    )
}
...
const Pipes:React.FC<PipeProps> = ({position, size, coordX=0}: PipeProps) => {
    ...
    return (
        <div className="pipes-container" style={{transform: `translate(${move}px)`,}}>
